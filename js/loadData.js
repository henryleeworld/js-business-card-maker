(function($) {
    ...
    dataLoader = {
        initiator : function(){
            $.getJSON('data/cards/cards.json', function (data) {
        ...
        loadAllCss : function(styles) {
            for(var i=0;i<styles;i++){
                $('head').append('<link rel="stylesheet" href="data/cards/'+cardData[i].style+'/'+cardData[i].style+'.css" type="text/css" />');
            }
        },
        loadDataToHtml : function(i) {
            $('.card__display-content').load('data/cards/'+cardData[i].style+'/'+cardData[i].style+'.html',function(){
                callTo.fillCardData();
